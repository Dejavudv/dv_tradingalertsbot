...
async def main():
    ...
    for index, row in df.iterrows():
        ...
        if not pd.isna(ema_50) and not pd.isna(ema_200):
            ...
            if abs(close_price - ema_50) / ema_50 < 0.01 and ema_50 > ema_200:
    ...
    # Iterate through the DataFrame to find points where the closing price equals the 50 EMA and the 50 EMA is below the 200 EMA
    for index, row in df.iterrows():
        close_price = int(row['close']) 
        ema_50 = row['ema_50']
        ema_200 = row['ema_200']
        ...
        if not pd.isna(ema_50) and not pd.isna(ema_200):
            ...
            if close_price < 0.98 * ema_50 and ema_50 < ema_200:
                latest_green_dot = index  # Update the latest green dot position

    # Check if a green dot was found and send the corresponding alert message
    if latest_green_dot:
        await send_message('1706992952', f"BUY ALERT! in {timeframe} timeframe\n------------------\nCurrent Price: {close_price}\n50 EMA: {ema_50}\n200 EMA: {ema_200}\n\n\n\n Position date: {latest_green_dot}")
    else:
    ...
    # Plot green dots
    plt.scatter(latest_green_dot, df.loc[latest_green_dot]['close'], color='red', label='buy alert')
